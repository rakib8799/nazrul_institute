...
<?php
if (isset($_SESSION['author_role'], $_SESSION['author_id'], $_SESSION['researcher_notice_id']) && $_SESSION['author_role'] === 'Student') {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_FILES['pdf_file']['name'])) {
            ...
            if (!in_array($path_info3, $arr3)) {
                ...
                $final_report_file = $_SESSION['researcher_final_report_file'];

                $select_from_new_paper = "SELECT * FROM `project_submission_student` WHERE researcher_author_id='$researcher_author_id' AND researcher_notice_id='$researcher_notice_id' AND researcher_final_report_file='$final_report_file'";
                ...
                if (mysqli_num_rows($run_select_from_new_paper) > 0) {
                    ...
                    if ($researcher_final_report_file != $_SESSION['researcher_final_report_file']) {
                    ...
                    // } 
                        ...
                        $update_sql = "UPDATE `project_submission_student` SET `researcher_final_report_file`='$pdf_file_name' WHERE researcher_author_id='$researcher_author_id' AND researcher_notice_id='$researcher_notice_id' AND  researcher_final_report_file='$researcher_final_report_file'";
...
?>
    ...
    <?php
                    ...
                    echo "<p class='text-danger text-bold text-center fs-5 mt-3'>আপনি ইতিমধ্যে চূড়ান্ত প্রতিবেদন জমা দিয়েছেন। তাই আপাতত আর কোনো প্রতিবেদন জমা দিতে পারবেন না।</p>";
                }
            }
        }
    }
    ...
    <?php
} else if (isset($_SESSION['author_role'], $_SESSION['author_id'], $_SESSION['advisor_notice_id']) && $_SESSION['author_role'] === 'Teacher') {
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_FILES['pdf_file']['name'])) {
            ...
            if (!in_array($path_info3, $arr3)) {
                ...
                $final_report_file = $_SESSION['advisor_final_report_file'];
                $select_from_new_paper = "SELECT * FROM `project_submission_teacher` WHERE advisor_author_id='$advisor_author_id' AND advisor_notice_id='$advisor_notice_id' AND advisor_final_report_file='$final_report_file'";
                $run_select_from_new_paper = mysqli_query($conn, $select_from_new_paper);
                if (mysqli_num_rows($run_select_from_new_paper) > 0) {
                    $row = mysqli_fetch_assoc($run_select_from_new_paper);
                    extract($row);
                    ...
                    if ($advisor_final_report_file != $_SESSION['advisor_final_report_file']) {
                        echo "<p style='height: 80vh;' class='text-danger text-bold text-center fs-3 mt-3 d-flex justify-content-center align-items-center'>আপনি ইতিমধ্যে প্রকল্পের চূড়ান্ত প্রতিবেদন জমা দিয়েছেন। তাই আপাতত আর কোনো প্রতিবেদন জমা দিতে পারবেন না।</p>";
                    } else {
                        $author_id = $_SESSION['author_id'];
                        $update_sql = "UPDATE `project_submission_teacher` SET `advisor_final_report_file`='$pdf_file_name' WHERE advisor_author_id='$advisor_author_id' AND advisor_notice_id='$advisor_notice_id' AND advisor_final_report_file='$advisor_final_report_file'";
                        $run_insert_qry = mysqli_query($conn, $update_sql);
                        if ($run_insert_qry) {
                            // move_uploaded_file($doc_file_tmp_name, '../Files/notices/doc_file/' . $doc_file_name);
...
// else if (!isset($_SESSION['researcher_notice_id']) || !isset($_SESSION['advisor_notice_id'])) {
