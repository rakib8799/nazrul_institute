   ...
   <div class="row">
      ...
      <div class="col-md-3 col-12 mt-2">
         <div class="card shadow p-3 mb-5" style="border-radius: 8px; height: 15vh;">
            ...
            <?php
            if (isset($_SESSION['author_role'], $_SESSION['author_email']) && $_SESSION['author_role'] === 'Student') {
               $author_email = $_SESSION['author_email'];
               // select paper information
               $select_from_new_paper = "SELECT * FROM `project_submission_student` WHERE `researcher_author_id` = '$_SESSION[author_id]' AND `paper_status` = '1' AND `researcher_email`='$author_email'";
               $run_select_from_new_paper = mysqli_query($conn, $select_from_new_paper);
               $num_new_paper = mysqli_num_rows($run_select_from_new_paper);
            ?>
               <a href="new_papers.php" class="home_anker">সর্বমোট প্রকল্প: <?php echo $obj->engToBn($num_new_paper) ?></a>
            <?php
            } else if (isset($_SESSION['author_role'], $_SESSION['author_email']) && $_SESSION['author_role'] === 'Teacher') {
               $author_email = $_SESSION['author_email'];

               $select_from_new_paper = "SELECT * FROM `project_submission_teacher` WHERE `advisor_author_id` = '$_SESSION[author_id]' AND `paper_status` = '1'  AND `advisor_email`='$author_email'";
               $run_select_from_new_paper = mysqli_query($conn, $select_from_new_paper);
               $num_new_paper = mysqli_num_rows($run_select_from_new_paper);
            ?>
               <a href="new_papers.php" class="home_anker">সর্বমোট প্রকল্প: <?php echo $obj->engToBn($num_new_paper) ?></a>
