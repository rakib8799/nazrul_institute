...
if (isset($_GET['id'], $_GET['pdf_file1'], $_GET['pdf_file2'])) {
    ...
    $deleteData = "DELETE FROM project_submission_student WHERE `id` = '$id'";
    $result = mysqli_query($conn, $deleteData);
    if ($result) {
        // unlink('../Files/project_submission_student/doc_file/' . $doc_file);
        unlink('../Files/project_submission_student/pdf_file/' . $pdf_file1);
        unlink('../Files/project_submission_student/pdf_file/' . $pdf_file2);
